...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/05 16:54:30 by albeninc          #+#    #+#             */
/*   Updated: 2024/07/05 16:58:23 by albeninc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void IRCServer::handleFileCommand(int clientSocket, const std::string& message) {
    ...
    if (chdir("../Downloads") != 0) {
        std::string errorMessage = getCommandPrefix(clientSocket) + "PRIVMSG " + target + " :Failed to change directory to Downloads\r\n";
        send(clientSocket, errorMessage.c_str(), errorMessage.length(), 0);
        return;
    }
    ...
    if (mkdir(directory.c_str(), 0777) != 0 && errno != EEXIST) {
        std::string errorMessage = getCommandPrefix(clientSocket) + "PRIVMSG " + target + " :Could not create received_files directory\r\n";
        send(clientSocket, errorMessage.c_str(), errorMessage.length(), 0);
        return;
    }
